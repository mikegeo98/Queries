...
spark = SparkSession.builder.appName("Query2_DFAPI").config("spark.executor.instances","5").config("spark.executor.cores","4").master("yarn").getOrCreate()
...
part = spark.read.format("csv").options(inferSchema = "true" , header = "false").schema(partSchema).option("delimiter", "|").csv("/user/diplomma/data/data50/part.tbl")
...
partsupp = spark.read.format("csv").options(inferSchema = "true" , header = "false").schema(partsuppSchema).option("delimiter", "|").csv("/user/diplomma/data/data50/partsupp.tbl")
